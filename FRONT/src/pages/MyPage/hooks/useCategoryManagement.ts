...
function useCategoryManagement() {
  ...
  const moveCategory = (sourceId: number, destinationId: number) => {
    ...
    const addCategoryToDestination = (categories: Category[], destinationId: number, categoryToAdd: Category): boolean => {
      for (const category of categories) {
        if (category.id === destinationId) {
          category.children = [...(category.children || []), categoryToAdd];
          return true;
        }
        if (addCategoryToDestination(category.children || [], destinationId, categoryToAdd)) {
          return true;
        }
      }
      return false;
    ...
    if (!addCategoryToDestination(updatedCategories, destinationId, sourceCategory)) {
      console.error("Destination category not found");
      return;
    }

    setCategories(updatedCategories);
    addPendingChange({ type: "update", payload: { categoryId: sourceId, parentId: destinationId } });
  };
